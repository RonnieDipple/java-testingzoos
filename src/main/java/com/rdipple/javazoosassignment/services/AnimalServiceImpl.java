...
public class AnimalServiceImpl implements AnimalService {
    ...
    public Animal findAnimalById(long id) {
        return animalRepo.findById(id).orElseThrow(() -> new ResourceNotFoundException("FindAnimalByIdFail"));
    }

    @Transactional
    @Override
    public void delete(long id) {
        animalRepo.findById(id).orElseThrow(() -> new ResourceNotFoundException("Animal delete fail"));
    ...
    public Animal save(Animal animal) {
        ...
        if (animal.getZooanimals().size() > 0) throw new ResourceFoundException("ZooAnimals not created: Animal");
    ...
    public Animal update(long id, Animal animal) {
        ...
        if (animal.getZooanimals().size() > 0) throw new ResourceNotFoundException("Zooanimals not updated: Animal");

        if (animalRepo.findById(id) != null) {
            animalRepo.updateAnimalType(id, animal.getAnimaltype());
        } else throw new ResourceNotFoundException("No animal with animalId " + id + " exists");
